...
Column cartItemsFounded(CartLoaded state ,BuildContext context) {
  ...
  double totalPrice = BlocProvider.of<CartCubit>(context).calculateTotalPriceInCart(state.items,context);
int numberOfItems =BlocProvider.of<CartCubit>(context).calculateTotalItemsInCart(state.items);
  return Column(
    children: [
      Expanded(
        flex: 4,
